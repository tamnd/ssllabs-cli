// Package cli assembles the ssllabs command tree from the ssllabs
...
// NewApp assembles the kit application from the ssllabs domain. The
...
//
// To add a command, declare it in ssllabs/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ssllabs.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ssllabs.Domain{}).Register(app)
